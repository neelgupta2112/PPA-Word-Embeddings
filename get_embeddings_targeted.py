...
df = df[["page_id", "form", "spelling", "page_text", "work_id", "title", "author", "pub_year"]]

# map form -> list of spellings
form_to_spellings = {
    f: [s.lower().strip(string.punctuation) for s in spellings]
    for f, spellings in df.groupby("form")["spelling"].agg(list).items()
}
